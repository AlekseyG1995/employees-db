...
export const useFiltersViewEmployees = (
  fetchEmployees: (filters: IFilters, preferCacheValue: boolean) => {}
) => {
  ...
  const changeParams = ({
    ...
    if (isArchive !== defaultFilters.isArchive)
      obj.isArchive = String(isArchive)
  ...
  const [filters, setFilters] = useState<IFilters>({
    isArchive:
      searchParams.has('isArchive') && searchParams.get('isArchive') === 'true'
        ? true
        : false || defaultFilters.isArchive,
    showOnPage: searchParams.has('limit')
      ? Number(searchParams.get('limit'))
      : defaultFilters.showOnPage,
